...
if [[ "${EUID}" -eq 0 ]]; then
  ...
  apt-get update
  apt-get install -y python3 python3-venv python3-pip python3-tk rsync
fi
...
if [[ "${EUID}" -eq 0 ]]; then
  ...
  sudo -u "$TARGET_USER" "$INSTALL_DIR/.venv/bin/pip" install -r "$INSTALL_DIR/redos_notifier/requirements.txt"
else
...
ln -sfn "$TARGET_HOME/.config/systemd/user/redos-notifier.service" "$TARGET_HOME/.config/systemd/user/default.target.wants/redos-notifier.service"

if [[ "${EUID}" -eq 0 ]]; then
  chown -R "$TARGET_USER:$TARGET_USER" "$TARGET_HOME/.config" "$TARGET_HOME/.local" || true
  chown -h "$TARGET_USER:$TARGET_USER" "$TARGET_HOME/.config/systemd/user/default.target.wants/redos-notifier.service" || true
  chown -R "$TARGET_USER:$TARGET_USER" "$TARGET_HOME/.local/bin/vacation-notifier" "$TARGET_HOME/.local/share/applications/vacation-notifier.desktop" "$TARGET_HOME/.config/autostart/vacation-notifier.desktop" "$TARGET_HOME/.config/systemd/user/redos-notifier.service" "$INSTALL_DIR/.env"
fi

if command -v loginctl >/dev/null 2>&1 && [[ "${EUID}" -eq 0 ]]; then
...
Notifier installed for user: ${TARGET_USER}
Launcher: ${TARGET_HOME}/.local/bin/vacation-notifier
Desktop entry: ${TARGET_HOME}/.local/share/applications/vacation-notifier.desktop
